...
export function CVTextarea({ value, onChange }: CVTextareaProps) {
  return (
    <div>
      <label htmlFor="cv-textarea" className="mb-2 block text-sm font-medium text-text-primary">Or paste your CV text</label>
      <textarea
        id="cv-textarea"
        data-testid="cv-textarea"
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="min-h-[160px] w-full resize-y rounded-[var(--radius)] border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] px-3 py-3 text-sm leading-relaxed text-text-primary placeholder:text-text-subtle outline-none transition-colors duration-200 focus:border-[hsl(var(--violet))] focus:shadow-[0_0_0_1px_hsl(var(--violet)/0.3)]"
