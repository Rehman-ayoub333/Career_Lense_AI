...
| Gemini API timeout/5xx | Upstream outage or slow response | Existing retry/backoff (`lib/ai/google.ts`), then surfaced as `AI_INVALID_OUTPUT`-equivalent honest failure, never an infinite spinner |
